...
class Home extends React.Component {
  ...
  showanswer = () => {
    ...
    for (var i = 1; i < possibleAns.length; i++) {
      if (
        possibleAns[i].value !== possibleAns[i - 1].value ||
        possibleAns[i].algr !== possibleAns[i - 1].algr
      ) {
    ...
    if (isSolvable) {
      solstr = solstr.slice(0, solstr.length - 5);
    }
  ...
  render() {
    return (
      <div>
        ...
        <p></p>
        {this.state.card.map((cardvalue, index) => {
          return (
            <input
              type="number"
              value={cardvalue}
...
function calculate(card) {
  ...
  if (card.length === 1) {
    ...
    for (i = 1; i < 2 ** card.length - 1; i++) {
      ...
      A = [];
      B = [];
      for (j = i.toString(2).length - 1; j >= 0; j--) {
        istr.unshift(i.toString(2)[j]);
      }
      for (j = i.toString(2).length; j < card.length; j++) {
        istr.unshift('0');
      }
